        ...
        return (
          <div className="flex flex-col space-y-2 relative" key={index}>
            ...
            <div className="relative left-10 w-full">
              ...
              <p className={`lg:text-xl text-lg font-normal overflow-hidden text-ellipsis ${longForm ? "h-auto" : "max-h-[4em]"}`}>
                {exp.description}
              </p>
              <p
                className="text-xl font-normal font-extrabold italic" 
                onClick={() => setLongForm(!longForm)}>
                  {longForm? "read less": "read more"}
              </p>
            </div>
